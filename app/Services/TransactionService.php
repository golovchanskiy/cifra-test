...
class TransactionService
{
     ...
     * @throws TransactionAlreadyExistsException
     */
    private function hold(string $transactionId, int $userId, float $amount): Transaction
    {
     ...
     * @throws TransactionInvalidException
     */
    private function complete(string $transactionId, int $userId, float $amount): Transaction
    {
        $transaction = $this->repository->find($transactionId, $userId);
    ...
    {
        $transaction = $this->cancel($dto->getId(), $dto->getUserId(), $dto->getAmount());

        if ($dto->getSenderUserId()) {
            $amount = -$dto->getAmount();

            $transaction = $this->cancel($dto->getId(), $dto->getSenderUserId(), $amount);
        }
    }
     ...
     * @throws TransactionInvalidException
     */
    private function cancel(string $transactionId, int $userId, float $amount): Transaction
    {
        $transaction = $this->repository->find($transactionId, $userId);
        ...
        // возвращаем деньги на счет если по холдированию было списание
        if ($amount < 0) {
